...
from agents.mappo_mode0c import MAPPOTrainerMode0c as MAPPOTrainer

CFG = {
    # ── Environment ──────────────────────────────────────────────────
    "n_vehicles":     10,    # ← CHANGE THIS for each run: 2, 3, 4, 5, 7, 10
    "n_subchannels":  5,
    ...
    # ── Result tracking ───────────────────────────────────────────────
    "config_label": "D_N10",
    "result_file":  "results/_d_rerun.json",
}
...
def move_to_device(buf: dict, device: torch.device) -> dict:
    ...
    return {
        ...
        "vehicle_idx":  torch.LongTensor(buf["vehicle_idx"]).to(device, non_blocking=True),
    }
...
def main():
    ...
    env     = V2XEnv(CFG)
    CFG["_vehicle_classes_for_logging"] = list(env.vehicle_classes)
    trainer = MAPPOTrainer(CFG, obs_dim, action_dim, global_state_dim)

    N = CFG["n_vehicles"]
    writer = SummaryWriter(f"logs/config_{CFG['config_label']}_N{N}_mode0c")

    print(f"Device          : {device}")
    total_actor_params = sum(sum(p.numel() for p in a.parameters()) for a in trainer.actors)
    print(f"Actors          : {len(trainer.actors)} per-vehicle, total {total_actor_params:,} params")
    ...
    sched_actors = [
        torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=CFG["n_episodes"], eta_min=CFG["lr_min"])
        for opt in trainer.opts_a
    ]
    ...
    for ep in range(n_ep):
        ...
        # ── PPO update ────────────────────────────────────────────────
        for k in buf:
            buf[k] = np.array(buf[k], dtype=np.float32 if k not in ("vehicle_idx","actions") else np.int64)
        ...
        writer.add_scalar("lr_actor",       sched_actors[0].get_last_lr()[0],     ep)
    ...
    result_record = {
        ...
        "architecture":      "mode_0c_per_vehicle_actors",
    }
    ...
    # ----- Save policy checkpoint (Phase D reproducibility package) -----
    ckpt_dir = pathlib.Path("checkpoints")
    ckpt_dir.mkdir(exist_ok=True)
    ckpt_path = ckpt_dir / (CFG["config_label"] + "_actors.pt")
    torch.save({
        "config_label":      CFG["config_label"],
        "N":                 CFG["n_vehicles"],
        "M":                 CFG["n_subchannels"],
        "M_m0":              CFG["m0_subchannel_count"] if CFG.get("demand_separation") else None,
        "demand_separation": CFG.get("demand_separation", False),
        "architecture":      "mode_0c_per_vehicle_actors",
        "actor_state_dicts": [a.state_dict() for a in trainer.actors],
